...
/// <summary>
/// Extension de <see cref="TeleportationArea"/> qui permet l'activation d'un fondu au noir lors de la téléportation.
/// </summary>
public class TeleportationAreaExtension : TeleportationArea { // TODO La téléportation peut bugger par moments !
	...
	private IEnumerator FadeCoroutine(UnityAction<SelectEnterEventArgs> actionEvent, SelectEnterEventArgs args) {
		...
		masterController.solver.locomotion.stepSpeed = 50;
		
		// Pour téléporter le corps en même temps, sinon il se déplaçait rapidement vers le joueur une fois la téléportation finie.
		//masterController.solver.AddPlatformMotion(args.interactorObject.transform.position, args.interactorObject.transform.rotation, args.interactorObject.transform.position);

		fadeScreen.FadeOut();
		yield return new WaitForSeconds(fadeScreen.fadeDuration);

        masterController.solver.locomotion.stepSpeed = 3;
